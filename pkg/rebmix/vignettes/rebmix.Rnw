...
\section{What's new in version \texttt{2.17.1}}

In version \texttt{2.17.1}, several improvements have been introduced. 
Additional methods for accelerating the EM algorithm have been implemented, including linear and quadratic acceleration schemes from \citep{varadhan2008acceleration}. 
These can be enabled by setting the \texttt{acceleration} slot of the \texttt{EM.control} class to the appropriate value. 
