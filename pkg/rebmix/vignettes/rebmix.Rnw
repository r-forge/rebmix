...
\section{What's new in version \texttt{2.14.0}}
Version \texttt{2.14.0} introduces several major improvements of the package. The package has been extended to three parameter parametric families. The Gumbel probability density
\begin{equation}
f(y) = \frac{1}{\sigma}\exp\left(\frac{y - \mu}{\xi \sigma} - \exp\left(\frac{y - \mu}{\xi \sigma}\right)\right).
\end{equation}
has been added as the first three parameter family type. Here, $\xi \in \{-1, \textrm{NA}, 1\}$. If initially $\xi$ is set to $\textrm{NA}$, the package estimates $\xi$. Otherwise the  estimated $\xi$ equals initial $\xi$.

Class \texttt{"EMMIX.Theta"} and method \texttt{EMMIX} are added. They enable parameter assessment for all parametric families available in the package by using the EM algorithm only.  
